...
function get_preview()
{
 ...
 var unit_types = document.forms['xForm'].elements['unit'];
  unit_rdio = getCheckedValue(unit_types);
	...
	if(tn == "1"){	from_unit_type = from_unit_type.replace(/"/g, "'");
		to_unit_type = to_unit_type.replace(/"/g, "'");}else{
		to_unit_type =1;
		from_unit_type =1;}
	//alert("farhan2");
...
function conversion_validate(){
	...
	var transaction_nature 		= this.document.getElementById('transaction_nature');
        var transaction_naturef          = this.document.getElementById('transaction_nature').value;

	var from_fund 				= this.document.getElementById('from_fund');
	//alert(from_fund); return false;
//	var types_of_units 			= this.document.getElementById('types_of_units');
	if(transaction_naturef == '1'){
	...
	if(from_unit_type  == null && transaction_naturef == '1'){
	...
	if(to_unit_type  == null && transaction_naturef == '1'){ 
	...
	if(from_fund.value  == 0)
	{
	 ...
	 this.document.getElementById('from_fund_error').style.display="";
	 return false;
	 }//alert("farhan1" + transaction_naturef);
/* farhan
	if(from_unit_type.value  == 0 && transaction_naturef == '1'){ //alert("farhan_in");
	this.document.getElementById('from_fund_types_of_units_error').style.display=""; 
	return false;
	}//alert("farhan");
*/
...
/* farhan
	if(to_unit_type.value  == 0 && transaction_naturef == '1'){ 
	this.document.getElementById('to_fund_types_of_units_error').style.display=""; 
	return false;
	}
*/
...
/*	if(transaction_type.value == '-1'){
		err_msg+= 'Missing Transaction Type\n';
		this.document.getElementById('transaction_type_error').style.display="";
	}*/
	
	if(transaction_nature.value == -1){
		err_msg+= 'Missing Transaction Nature\n';
		this.document.getElementById('transaction_nature_error').style.display="";
	}	
	//alert('hi');
	/*if(from_fund.value == '0' || from_fund.value == ''){
		err_msg+= 'Missing From Fund Type\n';
		this.document.getElementById('from_fund_error').style.display="";
		//flag =false;
	}
	alert('bye');
	if(from_unit_type.value == '0' || from_unit_type.value == ''){
		err_msg+= 'Missing From Unite Type\n';
		this.document.getElementById('from_fund_types_of_units_error').style.display="";
	}
	if(to_fund.value == '0' || to_fund.value == ''){
		err_msg+= 'Missing To Fund\n';
		this.document.getElementById('to_fund_error').style.display="";
	}
	if(to_unit_type.value == '0' || to_unit_type.value == ''){
		err_msg+= 'Missing To Unite Type\n';
		this.document.getElementById('from_fund_types_of_units_error').style.display="";
	}*/
...
if(isset($_REQUEST['add']) || isset($_REQUEST['edit']))
{
	...
	*/
	...
	//echo "farhan";exit();
	if($_REQUEST['from_fund_types_of_units'] == '0' and $_REQUEST['transaction_nature'] == '1'){
	...
	if($_REQUEST['to_fund_types_of_units'] == '0' and $_REQUEST['transaction_nature'] == '1'){
	...
	if($_REQUEST['unit_value'] == ''){
       ...
       // echo "farhan2". $from_fund . "__" .  $to_fund ;exit();
	if($flag == true){
...
//       echo "farhan2". $from_fund . "__" .  $to_fund ;exit();
  //     $from_fund = "5";
		...
		//       echo "farhan22". $from_fund . "__" .  $to_fund ;exit();
		  ...
		  //     echo "farhan12". $from_fund . "__" .  $to_fund ;exit();
