...
$(document).ready(function(){
	...
	$(".msg_head_F2").click(function(){
		$(this).next(".msg_body_F2").slideToggle('fast');

		var contentPanelId = jQuery(this).attr("id");
		var sentence = document.getElementById('txtquery2').value;
		if (sentence.indexOf(contentPanelId)==-1)
		{
			if(document.getElementById('txtquery2').value == '')
			{
				 document.getElementById('txtquery2').value+= contentPanelId ;
			}
			else
			{
				document.getElementById('txtquery2').value+= ',' + contentPanelId ;
...
<div class="msg_list">
 <?php
 	$count = 1;
     while(!$rs->EOF){ ?>
		<p class="msg_head_F2" id="F2-Q<? echo $count; ?>">
		<b>Q<? echo $count; ?>.</b>&nbsp; <?php echo $rs->fields["question"]; ?>
		</p>
		<div class="msg_body_F2">
