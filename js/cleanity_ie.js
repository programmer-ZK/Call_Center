$(function(){	
	...
	var navSelectorlist = "ul#menu-list li";/** define the main navigation selector **/
	...
	// $('.box-container').corners("5px bottom");
	 ...
	 $('textarea#email_body').wysiwyg();
	 
	 $("div#sys-messages-container a, div#to-do-list ul li a").colorbox({fixedWidth:"50%", transitionSpeed:"100", inline:true, href:"#sample-modal"}); 
 /** jquery colorbox modal boxes for system messages and to-do list - see colorbox help docs for help: http://colorpowered.com/colorbox/ **/
 
 	 $("a#sample-1").colorbox({fixedWidth:"50%", transitionSpeed:"100", inline:true, href:"#sample-modal-1"}); 
 /** jquery colorbox modal boxes for system messages and to-do list - see colorbox help docs for help: http://colorpowered.com/colorbox/ **/

 	 $("a#sample-2").colorbox({fixedWidth:"50%", transitionSpeed:"100", inline:true, href:"#sample-modal-2"}); 
 /** jquery colorbox modal boxes for system messages and to-do list - see colorbox help docs for help: http://colorpowered.com/colorbox/ **/


 	 $("a#sample-3").colorbox({fixedWidth:"50%", transitionSpeed:"100", inline:true, href:"#sample-modal-3"}); 
 /** jquery colorbox modal boxes for system messages and to-do list - see colorbox help docs for help: http://colorpowered.com/colorbox/ **/
	...
	jQuery(navSelectorlist).find('a').css( {backgroundPosition: "0 0"} );
...
jQuery(navSelectorlist).hover(function(){/** build animated dropdown navigation **/
		jQuery(this).find('ul:first:hidden').css({visibility: "visible",display: "none"}).show("fast");
		jQuery(this).find('a').stop().animate({backgroundPosition:"(0 -40px)"},{duration:150});
 	   jQuery(this).find('a.top-level').addClass("blue");
		},function(){
