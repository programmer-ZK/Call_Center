...
/****************************************hk***********************************************/		
		for ($i=1; $i<=4; $i++)
		{
			//Fields not used in this report:
			//Question_code, ROUND(SUM(ePoints) / SUM(Maxpoints) * 100) AS Score,
			$Week_query = "SELECT  SUM(IF(points_type='yes',Qcount,0)) AS Yes, SUM(IF(points_type='no',Qcount,0)) AS 'No', SUM(IF(points_type='nimp',Qcount,0)) AS nimp, ";
			...
			$num_rows2 = mysql_num_rows($week_result);
...
//				}
			...
			else
			{
				...
				{ 	
					...
					$n_imp += $week_row['nimp'];	
					//echo "QuestionRow[".$l."]". $QuestionRow[$l]."<br>";
					$l = $l + 1;
				}
			}	
			...
			$MonthTotal = $MonthTotal + $WeekSum[$i-1];
		...
		//echo "<br>week sum: ";print_r($WeekSum); echo "<br> month total = MonthTotal / ScoredWeeks = ".$MonthTotal." / ".$ScoredWeeks." = ".round($MonthTotal/$ScoredWeeks)."<br><br>";
		//echo "<br>week sum: ";print_r($WeekSum); echo "<br> month total = MonthTotal / ScoredWeeks = ".$MonthTotal." / 4 = ".round($MonthTotal/4)."<br><br>";
...
/************************************************hk**************************************/
		...
		{ 
				...
				for($j=1;$j<=4;$j++)
				{
					...
					if ($weekrow)
					{
						...
						if($j==$weekrow['WeekNo'])
						{
							...
							$iCol = $weekrow['Score']/10;
							
							for($k=1;$k<=10;$k++)
							{
								if($k<=$iCol){
									$html .=  "<td style='border-bottom:1px solid #E8E8DD;' align='left' bgcolor='#5B6366'>&nbsp;</td>";/*#CCCCCC,938_8_,#BCB".($k-1)."B".($k-1)."*/
								}else{
									$html .=  "<td>&nbsp;</td>";
								}
							}
			...
			// divide by number of weeks which are not null
			//$html = str_replace($row['question_code']."V", round($question_total/$ScoredWeeks), $html);
			
			$html = str_replace($row['question_code']."V", round($question_total/4), $html);
			//echo "qcode: ".$row['question_code']." qtotal: ".$question_total . " weekcount: ".$week_count." scoredweeks: ".$ScoredWeeks."<br/>";
			$question_total=0;
			$week_count = 0;
			$html .=  "</table>";
		...
		// divide by number of weeks which are not null
		//$html = str_replace($group_name."V", round($MonthTotal/$ScoredWeeks), $html); 
		
		// Divide by fixed number of weeks in this case 4
		$html = str_replace($group_name."V", round($MonthTotal/4), $html); 
/*********************************hk********************************************/
		// divide by number of weeks which are not null and sum all column section values			
		//$c_p_score = $c_p_score + ($MonthTotal/$ScoredWeeks);
		
		$c_p_score = $c_p_score + ($MonthTotal/4);
		//echo $c_p_score."<br>";
/*********************************hk********************************************/	
...
/*********************************hk********************************************/	
	$c_p_score_avg = round($c_p_score / $gname_count);
	//echo "c_p_score_avg = c_p_score / gname_count = ".$c_p_score." / ".$gname_count."<br>";
	//echo "c_p_score_avg = ".$c_p_score_avg."<br>";
	$html = str_replace("call_perf_score", round($c_p_score_avg)."%", $html);
/*********************************hk********************************************/		
	$html .=  "</td></tr></table>";
	echo $html;
