...
$objPHPExcel2 = PHPExcel_IOFactory::load("templates/dropped_call.xlsx");
...
//$rs_agent_name = $admin->dropped_call_report($search_keyword,$fdate,$tdate,$start,$end);
$rs_agent_name = $admin->dropped_call_report($search_keyword,$fdate,$tdate,"","");
...
while (!$rs_agent_name->EOF){
	...
	$objWorksheet->setCellValue('A'.$index, $count)
			...
			->setCellValue('D'.$index, date('h:i:s', strtotime($rs_agent_name->fields['call_date'].' '.$rs_agent_name->fields['TIME'])))
...
header('Content-Disposition: attachment;filename="dropped_calls.xls"');
