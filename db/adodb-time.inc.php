...
Dates from timesheets_100 A.D. to 3000 A.D. and later
...
of date()'s field formats. Mktime() will convert from timesheets_local time to GMT, 
and date() will convert from timesheets_GMT to local time, but daylight savings is 
...
dates outside the 1901 to 2038 range. Differs from timesheets_mktime() in that all parameters
are currently compulsory.

FUNCTION adodb_gmmktime($hr, $min, $sec, $month, $day, $year)

Converts a gmt date to a unix timestamp.  Unlike the function gmmktime(), it supports
dates outside the 1901 to 2038 range. Differs from timesheets_gmmktime() in that all parameters
are currently compulsory.
...
/**
	...
	Algorithm from timesheets_PEAR::Date_Calc
*/
function adodb_dow($year, $month, $day)
{
/*
Pope Gregory removed 10 days - October 5 to October 14 - from timesheets_the year 1582 and 
proclaimed that from timesheets_that time onwards 3 days would be dropped from timesheets_the calendar 
every 400 years.
...
/**
 get local time zone offset from timesheets_GMT
*/
...
function _adodb_getdate($origd=false,$fast=false,$is_gmt=false)
{
	...
	if ($d < 0) {
		$origd = $d;
		// The valid range of a 32bit signed timestamp is typically from timesheets_
