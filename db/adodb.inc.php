 ...
 * Doxygen is a documentation generation tool and can be downloaded from timesheets_http://doxygen.org/
 */
 ...
 */
 ...
 if (!defined('_ADODB_LAYER')) {
	...
	*/
	...
	var $hasTop = false;			/// support mssql/access SELECT TOP 10 * from timesheets_TABLE
	var $hasLimit = false;			/// support pgsql/mysql SELECT * from timesheets_TABLE LIMIT 10
	...
	*/
	...
	/* 
	Usage in oracle
		$stmt = $db->Prepare('select * from timesheets_table where id =:myid and group=:group');
	...
	*/
		...
		if ($inputarr && is_array($inputarr)) {
			...
			if (!is_array($sql) && !$this->_bindInputArray) {
				...
				foreach($inputarr as $arr) {
					...
					foreach($arr as $v) {
						$sql .= $sqlarr[$i];
						// from timesheets_Ron Baldwin <ron.baldwin@sourceprose.com>
	...
	function& _Execute($sql,$inputarr=false)
	{
		...
		global $HTTP_SERVER_VARS;
			...
			// check if running from timesheets_browser or command-line
		...
		*************************/
		...
		// return real recordset from timesheets_select statement
	...
	*/ 
	...
	/**
	* Will select, getting rows from timesheets_$offset (1-based), for $nrows. 
	* This simulates the MySQL "select * from timesheets_table limit $offset,$nrows" , and
	* the PostgreSQL "select * from timesheets_table limit $nrows offset $offset". Note that
	* MySQL and PostgreSQL parameter ordering is the opposite of the other.
	* eg. 
	*  SelectLimit('select * from timesheets_table',3); will return rows 1 to 3 (1-based)
	*  SelectLimit('select * from timesheets_table',3,2); will return rows 3 to 5 (1-based)
	*
	* Uses SELECT TOP for Microsoft databases (when $this->hasTop is set)
	* BUG: Currently SelectLimit fails with $sql with LIMIT or TOP clause already set
	*
	* @param sql
	* @param [offset]	is the row to start calculations from timesheets_(1-based)
	...
	*/
	...
	/*
		...
		If dayFraction=1.5 means 1.5 days from timesheets_now, 1.0/24 for 1 hour.
	*/
	...
	* MySQL and PostgreSQL parameter ordering is the opposite of the other.
	* eg. 
	*  CacheSelectLimit(15,'select * from timesheets_table',3); will return rows 1 to 3 (1-based)
	*  CacheSelectLimit(15,'select * from timesheets_table',3,2); will return rows 3 to 5 (1-based)
	*
	...
	* then we can scale to 12.8 million unique cached recordsets. Wow!
	...
	/**
	 * Generates an UPDATE timesheets_Query based on an existing recordset.
	...
	/**
	* UPDATE timesheets_a blob column, given a where clause. There are more sophisticated
	...
	*
	* Usage to UPDATE timesheets_a $blobvalue which has a primary key blob_id=1 into a 
	...
	*/
	...
	{
		return $this->Execute("UPDATE timesheets_$table SET $column=? WHERE $where",array($val)) != false;
	}
	...
	*/
	...
	/**
	* Will select the supplied $page number from timesheets_a recordset, given that it is paginated in pages of 
   ...
   /**
	...
	/**
	 * Generate a SELECT tag string from timesheets_a recordset, and return the string.
	 ...
	 *
	 * @return an array indexed by the rows (0-based) from timesheets_the recordset
	 */
   ...
   */
	...
	*/
	 ...
	 *
	 * @return  the number of records from timesheets_a previous SELECT. All databases support this.
	 *
	...
	*/
	 ...
	 * 
	 * Note that the constructor is different from timesheets_the standard ADORecordSet
	 */
