...
$objPHPExcel2 = PHPExcel_IOFactory::load("templates/outbound_calls.xlsx");
...
$end = $_REQUEST["end"];	
/*$rs_agent_name = $admin->out_bound_calls_report($search_keyword,$fdate,$tdate,$start,$end);*/
...
while (!$rs_agent_name->EOF){
	...
	$objWorksheet->setCellValue('A'.$index, $count)
			...
			->setCellValue('G'.$index, $rs_agent_name->fields['remarks']);
			//->setCellValue('G'.$index, $rs_agent_name->fields['call_type'].' '.$rs_agent_name->fields['call_status']);			
...
header('Content-Disposition: attachment;filename="outbound_calls.xls"');
