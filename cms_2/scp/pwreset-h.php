            ...
            if (($staff=Staff::lookup($_POST['userid']))) {
                if (!$staff->hasPassword()) {
                    $msg = __('Unable to reset password. Contact your administrator');
					 Http::redirect('../pwreset.php?msg='.$msg);
                }
                elseif (!$staff->sendResetEmail()) {
					$msg = __('A password reset email was sent to the email on file for your account.  Follow the link in the email to reset your password.');
					 Http::redirect('../../pwreset.php?msg='.$msg);
...
elseif ($_GET['token']) {
    ...
    if (($id = $_config->get($_GET['token']))
        ...
        //$tpl = 'pwreset.login.php';
        Http::redirect('../../pwreset.php?token='.$_GET['token']);
    else
