    ...
    static function fromVars($vars, $update=false) {
        ...
        if (!$user) {
            ...
            $user = User::create(array(
                'register_num' => Format::htmldecode(Format::sanitize($vars['register_num'], false)),
                'name' => Format::htmldecode(Format::sanitize($name, false)),
                'cmid' => Format::htmldecode(Format::sanitize($vars['cmid'], false)),
                'uin' => Format::htmldecode(Format::sanitize($vars['uin'], false)),
                'cnic' => Format::htmldecode(Format::sanitize($vars['cnic'], false)),
                'phone' => Format::htmldecode(Format::sanitize($vars['phone'], false)),
           ...
           /* if($vars['i_type']=='CMID'){
             $user->set('cmid', Format::htmldecode(Format::sanitize($vars['cmid_uin'], false)));
            }
            if($vars['i_type']=='UIN'){
             $user->set('uin', Format::htmldecode(Format::sanitize($vars['cmid_uin'], false)));
            }  */
            ...
            foreach ($this->getDynamicData() as $cd) {
                ...
                if(!$data
                    ...
                    foreach ($cd->getFields() as $f) {
                        ...
                        elseif ($f->get('name') == 'cnic')
                            $f->value = $this->getCNIC();
                        elseif ($f->get('name') == 'register_num')
                            $f->value = $this->getRegiserNum();
                        elseif ($f->get('name') == 'cmid')
                            $f->value = $this->getCMID();
                        elseif ($f->get('name') == 'uin')
                            $f->value = $this->getUIN();
                       /* elseif ($f->get('name') == 'mobile')
                            $f->value = $this->getMobile();*/
                    }
                }
    ...
    function updateInfo($vars, &$errors, $staff=false) {
        ...
        foreach ($forms as $cd) {
            if (($f=$cd->getForm()) && $f->get('type') == 'U') {
                ...
                if (($cnic = $f->getField('cnic'))) {
                    $this->cnic = $cnic->getClean();
                }
                if (($register_num = $f->getField('register_num'))) {
                    $this->register_num = $register_num->getClean();
                }
                if (($phone = $f->getField('phone'))) {
                    $this->phone = $phone->getClean();
                }
                
                if (($cmid = $f->getField('cmid'))) {
                      $this->cmid = $cmid->getClean();
                }
                if (($uin = $f->getField('uin'))) {
                      $this->uin = $uin->getClean();
                }
                    //$this->cmid = $phone->getClean();                
