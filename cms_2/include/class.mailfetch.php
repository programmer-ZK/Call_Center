...
class MailFetcher {
     ...
     * is completely undefined. If Ticket is corrupting your email
    ...
    function run() {
        ...
        if(!function_exists('imap_open')) {
            $msg=_S('Ticket requires PHP IMAP extension enabled for IMAP/POP3 email fetch to work!');
        ...
        while (list($emailId, $errors)=db_fetch_row($res)) {
            ...
            if ($fetcher->connect()) {
                ...
                if (++$errors>=$MAXERRORS) {
                    ...
                    $msg="\n"._S('Ticket is having trouble fetching emails from the following mail account').": \n".
