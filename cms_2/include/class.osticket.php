...
class osTicket {
    ...
    function alertAdmin($subject, $message, $log=false) {
        ...
        if($email) {
            ...
            Mailer::sendmail($to, $subject, $message, '"'.__('Alerts').sprintf('" <%s>',$to));
        }
    ...
    /**
     * Fetch the current version(s) of Ticket softwares via DNS. The
     ...
     */
     function getLatestVersion($product='core', $major=null) {
        $records = dns_get_record($product.'.updates.ticket.com', DNS_TXT);
        if (!$records)
            return false;
    ...
    static function get_root_path($dir) {
         ...
         * inside the DOCUMENT_ROOT. This is most likely useful if Ticket
         * is run using something like Apache UserDir setting where the
         * DOCUMENT_ROOT of Apache and the installation path of Ticket
         * have nothing in comon.
         *
         * +---------------------------+-------------------+----------------+
         * | PHP Script                | SCRIPT_NAME       | ROOT_PATH      |
         * +---------------------------+-------------------+----------------+
         * | /home/u1/www/ticket/... | /~u1/ticket/... | /~u1/ticket/ |
         * +---------------------------+-------------------+----------------+
         *
