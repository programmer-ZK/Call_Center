...
if(isset($_REQUEST['submitwc'])){
  ...
  if(empty($cb) || $_REQUEST['echoSelection3']==""){
...
//	print_r($cb);exit;
        $flag = true;
        $outgoing_wc = " No Answer,Busy, Not Connected, Powered Off, Connected, Not Answering, CALL Later, NOT Available, Busy/Waiting, NOT Contacted";
        $outgoing_wc2 = " Connected,Potential,Not Interested";
    for($i=1; $i < $N; $i++){
          $workcodes =$cb[$i];
  //echo $workcodes;
...
?>
 ...
 //      header ("Location: exit.php");
     ...
     //   $tools_admin->update_cdr($unique_id, $agent_id); //Commmented as on 27-06-2013 all done in cc_queue_stats table
  }
}
?>
<div class="box">
        ...
        <h4 class="white"><?php echo "Set Work Codes" ;?></h4>
                ...
                <form action="<?php  echo $_SERVER['PHP_SELF'] ?>" method="post" class="middle-forms cmxform" name="xForm" id="xForm">
                                DESCRIPTION <input name="value_desc" id="value_desc" class="txtbox-short" style="width:300px;" value="<?php echo @$_REQUEST["value_desc"];?>" />
