            ...
            if(($note=$ticket->postNote($vars, $errors, $thisstaff))) {
               ...
               // print_r($errors['err']);
                ...
                if($errors['err'])
					 ...
					 $query="UPDATE ts_ticket SET isanswered ='1',status_id ='".$vars['note_status_id']."' ,updated ='".date('Y-m-d H:i:s')."',lastresponse ='".date('Y-m-d H:i:s')."'  where ticket_id=".$vars['id'];
					 db_query($query); 
                     $msg=__('Reply posted successfully');
                  //$errors['postnote'] = __('Unable to post reply. Correct the error(s) below and try again!');
            }
            break;
        case 'edit':
        case 'update':
...
if($thisstaff->getId()!="1"){
if(($teams=$thisstaff->getTeams()) && count(array_filter($teams)))
 $select=' select count(*) num FROM '.TICKET_TABLE.' ticket ' ;
 $select .= ' LEFT JOIN '.TICKET_STATUS_TABLE. ' status
            ON (status.id = ticket.status_id) ';  
 $select .= ' LEFT JOIN '.TEAM_TABLE.' team ON (ticket.team_id=team.team_id) ';			
 $select .=' WHERE (ticket.team_id IN ('.implode(',', db_input(array_filter($teams)))
            .') AND status.state IN("closed") ';
  $select .= ' )';
$ds = db_query($select);
//print $ds;
$num_rows = db_fetch_array($ds);
//$num_rows['num'];
//echo mysql_num_rows($ds);	
}
if($thisstaff->showAssignedOnly() && $stats['closed']) {
    $nav->addSubMenu(array('desc'=>__('My Closed Tickets').' ('.number_format(($num_rows['num'])? $num_rows['num']:$stats['closed']).')',
    ...
    $nav->addSubMenu(array('desc' => __('Closed').' ('.number_format(($num_rows['num'])?$num_rows['num']:$stats['closed']).')',
