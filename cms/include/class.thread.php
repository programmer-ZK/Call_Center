    ...
    */
    ...
    function getAttachmentsLinks($file='attachment.php', $target='', $separator=' ') {
        ...
        foreach($this->getAttachments() as $attachment ) {
            ...
            $str.=sprintf('<a class="Icon file no-pjax" href="%s" target="%s">%s</a>%s&nbsp;%s',
                    '../ticketfiles/'.Format::htmlchars($attachment['name']), $target, Format::htmlchars($attachment['name']), $size, $separator);
...
*/        }
    ...
    function postEmail($mailinfo) {
        ...
        // this Ticket installation. If this incoming mail matches the
    ...
    function lookupByEmailHeaders(&$mailinfo, &$seen=false) {
        ...
        foreach (array('in-reply-to', 'references') as $header) {
            ...
            foreach (array_reverse($matches[0]) as $mid) {
                ...
                // Ticket, the Mailer class can break it apart. If it came
    ...
    function create($vars) {
        ...
        $sql=' INSERT INTO '.TICKET_THREAD_TABLE.' SET created=NOW() '
			...
			.' ,status='.db_input($vars['note_status_id'])
			.' ,is_overdue='.db_input($vars['isoverdue'])
			.' ,team_id='.db_input($vars['old_team_id'])
			.' ,due_date='.db_input($vars['due_date'])
