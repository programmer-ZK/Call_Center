 ...
 * require a redirect to a remote site and a redirect back to Ticket in
...
class osTicketAuthentication extends StaffAuthenticationBackend {
    ...
    function authenticate($username, $password) {
        if (($user = new StaffSession($username)) && $user->getId() &&
            ...
            $sql='UPDATE '.STAFF_TABLE.' SET is_logged_in=1,lastlogin=NOW() ';
            if($user->isPasswdResetDue() && !$user->isAdmin())
                $sql.=',change_passwd=1';
            $sql.=' WHERE staff_id='.db_input($user->getId());
            db_query($sql);
            $sql  = "insert into ts_login_activity ";
			$sql .= "( staff_id, login_datetime,status, update_datetime) ";
			$sql .= " values ('".db_input($user->getId())."',NOW(), '2',NOW())";
			 db_query($sql);
            return $user;
        }
    }
