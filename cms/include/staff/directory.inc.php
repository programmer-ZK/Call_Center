...
$from='FROM '.STAFF_TABLE.' staff '.
      'inner JOIN '.DEPT_TABLE.' dept ON(staff.dept_id=dept.dept_id) ';
	   if($thisstaff->getId()!=="1"){
		 $from .='AND dept.manager_id ="'.$thisstaff->getId().'" '; 
		  }
...
if($res && ($num=db_num_rows($res)))
    ...
    $showing=__('No agents found! Or You Do not have permissions');
?>
     ...
     <tr>
        <td colspan="6">
            <?php if($res && $num) {
                ...
                ?>
            <?php } else {
                echo __('No agents found! Or You Do not have permissions!');
            } ?>
        </td>
     </tr>
    </tfoot>
