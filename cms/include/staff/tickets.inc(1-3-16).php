...
if($thisstaff->getId()!="1" && $status=="closed"){
  
if(($teams=$thisstaff->getTeams()) && count(array_filter($teams)))
    $qwhere =' WHERE (ticket.team_id IN ('.implode(',', db_input(array_filter($teams)))
...
$qfrom=' FROM '.TICKET_TABLE.' ticket '.
	   ...
	   ' LEFT JOIN cc_admin  agent ON agent.admin_id = ticket.who_created'.
...
<div id='basic_search'>
    ...
    <table>
        <tr>
            ...
            <td>&nbsp;&nbsp;<a href="#" id="go-advanced"><input type="button" class="button" value="<?php echo __('Advanced'); ?>"><!-- <b>[<?php //echo __('advanced'); ?>]<b/> --></a>&nbsp;<i class="help-tip icon-question-sign" href="#advanced"></i></td>
        </tr>
    </table>
    </form>
</div>
...
<div>
        <div class="pull-left flush-left">
            <h5><a href="<?php echo Format::htmlchars($_SERVER['REQUEST_URI']); ?>"
                title="<?php echo __('Refresh'); ?>"><i class="icon-refresh"></i> <?php echo
                $results_type.$showing; ?></a></h5>
        </div>
                ...
                if($showassigned) {
                    if($row['staff_id']){
                        ...
                        $lc=sprintf('<span class="Icon teamAssigned">%s</span>',Format::truncate($row['team'],40));
						 if(!strcasecmp($row['state'],'closed')){
						  echo $row['state'];	 
						 $query="SELECT	poster FROM ts_ticket_thread WHERE ticket_id ='".$row['ticket_id']."' ORDER BY id  DESC LIMIT 1";
					     $t = db_query($query);
						 $ds = db_fetch_array($t);
						 $lc=sprintf('<span class="Icon teamAssigned">%s</span>',Format::truncate($ds['poster'],40));
						}
                    }else{
                        $lc="  ";
					    }
