...
function getPriorityById($id){
	$select = 'SELECT prior.priority_desc from '.PRIORITY_TABLE.' as prior where prior.priority_id='.$id;
...
if ($ticket_id){
    $where = 'WHERE ticket.ticket_id = '.$ticket_id;
         ...
         //   echo TicketStatus::status_options();
          ...
          /*  if ($ticket->isOpen()
           ...
           <!-- <span class="action-button pull-right" data-dropdown="#action-dropdown-print">
			...
			<span class="action-button pull-right hideme" >
                <i class="icon-caret-down pull-right"></i>
                <a id="a-ticket-print" href="#"><i class="icon-print"></i> <?php
                    echo __('Print'); ?></a>
            </span>
...
<?php /*?><?php           if($thisstaff->canBanEmails()) {
                     if(!$emailBanned) {?>
...
<?php $ticket_data =  getTicketById($ticket->getId()); $ticket_data = db_fetch_array($ticket_data);?>
<table class="ticket_info" cellspacing="0" cellpadding="0" width="940" border="0">
    <tr>
        <td width="50%">
            <table border="0" cellspacing="" cellpadding="4" width="100%" style="border-spacing: 5px;">
                <tr>
                    <th width="100"><?php echo __('Status');?>:</th>
                    <td><?php echo $ticket->getStatus(); ?></td>
                </tr>
                <tr>
                    <th><?php echo __('Priority');?>: <?php $p = getPriorityById($ticket_data['priority_id']);?></th>
                    <td><?php echo $p['priority_desc'];//$ticket->getPriority(); ?></td>
                </tr>
                <tr>
                    <th><?php echo __('Product');?>:</th>
                    <td><?php echo Format::htmlchars($ticket_data['product']); ?></td>
                </tr> 
                <tr>
        ...
        <td width="50%" >
            <table border="0" cellspacing="" cellpadding="4" width="100%" style="border-spacing: 5px;" >
                <tr>
                    ...
                    <td><?php echo Format::htmlchars($ticket->getName());
                        ?>
                    </td>
                </tr>
                <tr>
                ...
                <td> <?php echo ($ticket_data['nature']=="Others")? $ticket_data['nature'].':'.$ticket_data['others']:$ticket_data['nature'];?></td>
                </tr>
                <tr>
                    <th><?php echo __('Initiator'); ?>:</th>
                    <td><?php echo Format::htmlchars($ticket_data['full_name']);?>
                    </td>
                </tr>
            </table>
        </td>
...
<table class="ticket_info" cellspacing="0" cellpadding="0" width="940" border="0">
    <tr>
        <td width="50%">
            <table cellspacing="0" cellpadding="4" width="100%" border="0"  style="border-spacing: 5px;">
                <?php
                if($ticket->isOpen()) { ?>
                <tr>
                ...
                <tr>
                    ...
                    <td>
						...
						$query="SELECT	poster FROM ts_ticket_thread  WHERE ticket_id ='".$ticket->getId()."' AND status='3' ORDER BY id  DESC LIMIT 1";
						$t = db_query($query);
						$ds = db_fetch_array($t);
                            echo '<span class="faded">&mdash; '.$ds['poster'].' &mdash;</span>';
                        ?>
                    </td>
                </tr>
                <?php
...
<div id="ticket_thread">
	...
	$threadTypes =array('M'=>'message','N'=>'note','A'=>'Assign','E'=>'Edit','R'=>'Reply','SA'=>'Self Assigned','SC'=>'Status Changed','RA'=>'Re Assigned');
    /* -------- Messages & Responses & Notes (if inline)-------------*/
    $types = array('M', 'R', 'N','A','E','RA','SA','SC');
...
<?php if($errors['err'] && $errors['err']!="EndUser email address is not valid! Consider updating it before responding") { ?>
...
<div id="response_options">
    <ul class="tabs">
        ...
        <li><a id="note_tab" href="#note"><?php echo __('Post Reply');?></a></li>
        <?php
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="3">
            ...
            <tr>
                <td width="120" style="vertical-align:top">
                 ...
                 <label><strong><?php echo __('Post Reply'); ?>:</strong><span class='error'>&nbsp;*</span></label>
                </td>
                <td>
                    <div>
                        <div class="faded" style="padding-left:0.15em"><?php
                        echo __('Post title - summary of the post '); ?></div>
                    ...
                    <textarea name="note" id="internal_note" cols="80"
                        placeholder="<?php echo __('Reply Detail'); ?>"
            ...
            <tr>
                ...
                <td>
                    ...
                    <input type="hidden" name="old_team_id" value="<?php echo $ticket->getTeamId(); ?>">
                    <input type="hidden" name="due_date" value="<?php echo $ticket->getEstDueDate(); ?>">
                    <input type="hidden" name="priority_id" value="<?php echo $ticket_data['priority_id']; ?>">
