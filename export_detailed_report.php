...
//print_r($_REQUEST);
//die;
  $rs_agent_name = $admin->get_agent_detailed_report($_REQUEST['search'],date('Y-m-d', strtotime($_REQUEST['fdate'])),date('Y-m-d', strtotime($_REQUEST['tdate'])),"","");
//  print_r($_REQUEST);
// die; 
...
if($i==0){
if(!empty($_REQUEST['search'])){
	$objWorksheet->setCellValue('B2',$rs_drop_name->fields['full_name'])
			->setCellValue('B3', date('M-d-Y',strtotime($_REQUEST['fdate']))." To ".date('M-d-Y',strtotime($_REQUEST['tdate'])))
			->setCellValue('B4', $rs_drop_name->fields['department']);
}
...
//$rs_in_bound = $admin->get_agent_in_detailed($rs_agent_name->fields['staff_id'],$rs_agent_name->fields['update_datetime']);
