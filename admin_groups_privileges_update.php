...
<?php
	$page_name = "admin_groups_privileges_update";
	$page_title = "Admin Group Privileges Update";
	$page_level = "2";
	$page_group_id = "1";
	$page_menu_title = "Admin Group Privileges Update";
?>
...
<?php
	...
	$group_name		= $rsGroups->fields['group_name'];
	$page_id		= $_REQUEST['page_id'];
	$rights 		= $_REQUEST['rights'];
	
	if(isset($_REQUEST["update"]) && !empty($_REQUEST["update"])){
			$admin->delete_admin_privileges(0,$page_group_id,$group_id,0);
			$page_count = $_REQUEST['page_count'];
			$count = 0;
			while($count <> $page_count){
				$admin->insert_admin_privileges($page_id[$count],$page_group_id,$group_id,$rights[$count]);
				$count++;
			}
			$_SESSION[$db_prefix.'_GM'] = "[".$group_name."] admin group privileges updated successfully.";
			header ("Location: admin_groups_privileges_update.php?group_id=".$tools_admin->encryptId($group_id));
			exit();
		}
?>
        <div class="box">
        ...
        <div class="box-container">
                <form action="<? echo $_SERVER['PHP_SELF'] ?>" method="post" class="middle-forms" name="xForm">
					<input type="hidden" name="group_id" id="group_id" value="<?php echo $tools_admin->encryptId($group_id); ?>" />
                        <h3>Change / Update</h3>
                        <p>Please complete the form below. Mandatory fields marked <em>*</em></p>
							<fieldset>
									<legend>Fieldset Title</legend>
									<ol>
											<li class="even"><label class="field-title">Page Group <em>*</em>:</label> <label><?php echo $tools_admin->getcombo("pages_groups","page_group_id","group_id","group_name",$page_group_id,false,"","javascript:document.xForm.submit();"); ?><span class="form-confirm-inline" style="display:none;">Confirm Message</span></label><span class="clearFix">&nbsp;</span></li>
							</ol>
							</fieldset>
							<p class="align-right">
                ...
                <table class="table-short">
                        <thead>
							...
							<tr>
								...
								<td class="col-head2">Privillages</td>
							</tr>
                        </thead>
						<tfoot>
							<tr>
								<td colspan="4">
									<!--<a class="button" href="#"><span>perform action</span></a>-->
								</td>
							</tr>
      					</tfoot>
                    <?php
                        ...
                        <tbody>
                                <tr>
                            		...
                            		<td class="col-first">
                            		<?php 
						$checked = $tools_admin->returnFieldVal("Allow", "wot_admin_privileges", "Page_ID=".$rs->fields["page_id"]." and Page_Group_ID=".$page_group_id." and Group_ID=".$group_id);
			 			if($checked=="1"){
