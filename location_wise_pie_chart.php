...
<?php
        $page_name = "location_wise_pie_chart.php";
        $page_level = "0";
        $page_group_id = "0";
        $page_title = "Location Wise Pie Chart";
        $page_menu_title = "Location Wise Pie Chart";
?>
...
<?php $rs_agent_name = $admin->location_wise_pie_chart($fdate,$tdate);
$location_wise_pie_chart="";
...
<!-- ******************************  Agent On Call and Busy Times ************************** -->
	...
	<table class="table-short" id="keywords">
      			<thead>
      				<!--<tr>
	        	     <th colspan="3"><center><h4>Location Wise Pie Chart</h4></center></th>
					</tr>-->
                ...
                <td>
                 ...
                 <?php $arr =array('K'=>'Karachi','L'=>'Lahore','I'=>'Islamabad'); ?>
                <?php while(!$rs_agent_name->EOF){  ?>
                <?php  $location_wise_pie_chart .= "['".trim($arr[$rs_agent_name->fields['location']])."',".($rs_agent_name->fields['rec'])."]," ?>  
                <?php 
				$rs_agent_name->MoveNext();
				}   ?>
                  <div id="location_wise_pie_chart"></div>    
                </td>
                </tr>
                 </tr>
...
$(document).ready(function(){
	$('#a-ticket-print').click(function(){
		$('.hideme').hide();
	$('#location_wise_pie_chart').show().printElement();	
	$('.hideme').show();
	});
...
anychart.onDocumentReady(function() {
  ...
  chart = anychart.pie([<?php echo $location_wise_pie_chart;?>]);
  // set container id for the chart
  chart.container('location_wise_pie_chart');
  // set chart title text settings
  chart.title('Location Wise Pie Chart');
  //chart.credits().enabled(true);
  //chart.credits().logoSrc('');
