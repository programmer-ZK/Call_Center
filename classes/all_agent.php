...
class all_agent
{
    ...
    function get_agent_on_call_hold_times2($admin_id, $date, $type)
    {
        global $db_conn;
        global $db_prefix;
        $sql = "SELECT
        CASE WHEN start_datetime IS NOT NULL 
            THEN SEC_TO_TIME(SUM(TIME_TO_SEC(TIMEDIFF(end_datetime,start_datetime))))
            ELSE '00:00:00'
        END
        AS holdtime 

        FROM " . $db_prefix . "_vu_hold_time
        WHERE 1=1 ";
        if ($admin_id != 0) {
    ...
    function get_agent_on_call_hold_times($admin_id, $date, $type)
    {
...
WHERE 1=1 ";
    ...
    function get_agent_work_times($admin_id, $date)
    {
        ...
        global $db_conn;
        global $db_prefix; //echo $admin_id;exit;                
        ...
        //	  $sql.= "GROUP BY login_datetime order by staff_id";			  
        // echo $sql;//exit;
        $rs = $db_conn->Execute($sql);
        return $rs;
    }
    ...
    function get_agent_work_times_sum($admin_id, $date)
    {
        ...
        $sql = "SELECT
 CASE TIME(MAX(login_datetime)) WHEN TIME(MAX(logout_datetime))
  THEN TIMEDIFF(NOW(), MIN(login_datetime))
  ELSE TIMEDIFF(MAX(logout_datetime),MIN(login_datetime)) END AS duration
...
WHERE 1=1 ";
    ...
    function get_agent_abandon_calls2($admin_id, $date)
    {
        ...
        global $db_conn;
        global $db_prefix;
        $sql = "SELECT count(*) as trec FROM " . $db_prefix . "_abandon_calls WHERE 1=1 ";
        $sql .= " AND staff_id ='" . $admin_id . "'";
        $sql .= " AND DATE(update_datetime)='" . $date . "' ";
        //echo $sql;exit;
        $rs = $db_conn->Execute($sql);
