...
//four categroies of questions
    for($iLoop=1;$iLoop<=4;$iLoop++)
    {
		...
		if ($result) echo "</table></td>";
			...
			for ($i=1; $i<=4; $i++)
			{
				echo "<td align='center'><table border='1px'>";
				//echo "<tr><td>Week " . $i . "</td></tr>";
		
				$Week_query = "SELECT Question_code, ROUND(SUM(ePoints) / SUM(Maxpoints) * 100) AS Score, SUM(IF(points_type='yes',1,0)) AS Yes, SUM(IF(points_type='no',1,0)) AS 'No', SUM(IF(points_type='nimp',1,0)) AS nimp, ";
				$Week_query = $Week_query . "SUM(IF(points_type='na',1,0)) AS NA FROM cc_vu_evaluation WHERE evaluate_agent_id = '" . $agent_id . "' AND edate BETWEEN '".$sdate."' AND '".$edate."' ";
				...
				echo "<tr><td ><font size='2' face='Verdana, Arial, Helvetica, sans-serif'>Yes</font></td><td><font size='2' face='Verdana, Arial, Helvetica, sans-serif'>No</font></td><td><font size='2' face='Verdana, Arial, Helvetica, sans-serif'>N.Imp</font></td><td><font size='2' face='Verdana, Arial, Helvetica, sans-serif'>Score</font></td></tr>";
