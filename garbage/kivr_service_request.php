...
<?php
		...
		if(isset($_REQUEST['update']) || isset($_REQUEST['update']))
		{
			...
			$params = array('AccessKey' => $access_key,'Channel' => $channel,'AccountNo' => $account_no,'CustomerId' => $customer_id,'Residence' => $residence,'Office' => $office,'Mobile' => $mobile,'IsResidenceRegister' => $is_residence,'IsOfficeRegister' => $is_office,'IsMobileTransRegister' => $is_mobile,'IsMobileSmsRegister' => '', 'OperatingType' => $_SESSION[$db_prefix.'_OperatingTypeCode'], 'CallerId' => $caller_id);
			//print_r($params);exit;
		...
		while($count != count($rs1)){
			...
			$residence_html .= '<input type="radio" name="residence" id="residence" value="'.$rs1[$count]["Residence"].'"'.$checked_str.' onClick=check(this) /> '.$rs1[$count]["Residence"].'<br />';
			...
			$office_html .= '<input type="radio" name="office" id="office" value="'.$rs1[$count]["Office"].'"'.$checked_str.'" onClick=check(this) /> '.$rs1[$count]["Office"].'<br />';
			...
			$mobile_html .= '<input type="radio" name="mobile" id="mobile" value="'.$rs1[$count]["Mobile"].'"'.$checked_str.'" onClick=check(this) /> '.$rs1[$count]["Mobile"].'<br />';
			$count++;
		}
?>
