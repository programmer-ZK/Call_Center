...
$objPHPExcel2 = PHPExcel_IOFactory::load("templates/call_center_crm_summary_report_v2.xlsx");
...
/*$rs = $admin->callcenter_crm_report($_REQUEST['start'],$_REQUEST['end'],
date('Y-m-d', strtotime($_REQUEST['fdate'])),
date('Y-m-d', strtotime($_REQUEST['tdate'])),
$_REQUEST['tproduct'],$_REQUEST['tpriority'],$_REQUEST['tstatus'],
...
//print_r($_REQUEST); die;
$rs = $admin->callcenter_crm_report_v2("","",
date('Y-m-d', strtotime($_REQUEST['fdate'])),
date('Y-m-d', strtotime($_REQUEST['tdate'])),
$_REQUEST['tproduct'],$_REQUEST['tpriority'],$_REQUEST['tstatus'],
$_REQUEST['agent_id'],$_REQUEST['location'],$_REQUEST['e_natures'],$_REQUEST['intime'],$_REQUEST['depart_id'],$_REQUEST['sec'],$frdate,$trdate,$_REQUEST['cmp_id']);
...
//print_r($rs->fields);die('here');
$i=0;
while (!$rs->EOF){
	$depart =""; 
	if($rs->fields['dept_name']){
	 $depart = explode(" ",$rs->fields['dept_name']);  }else{
	...
	if($i==0){
		...
		$objWorksheet->setCellValue('D3',date('M-d-Y',strtotime($_REQUEST['fdate'])))
			 ...
			 ->setCellValue('C7',($_REQUEST['tpriority'])?$rs->fields['priority_desc']: "ALL")
			 ->setCellValue('C8',($_REQUEST['tstatus'])?$rs->fields['name']: "ALL")
	...
	$i++;
	if($rs->fields['status_id']==2 || $rs->fields['status_id']==3 ){
...
$res_duration ="";
 if($rs->fields['last_agent_response']){  //&& $rs->fields['action_type']!="E"
...
header('Content-Disposition: attachment;filename="call_center_crm_summary_report.xls"');
